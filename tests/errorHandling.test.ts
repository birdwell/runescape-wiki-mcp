...
describe('Error Handling', () => {
    ...
    describe('API Error Responses', () => {
        it('should handle 404 errors for price tools', async () => {
            ...
            await expect(handlePriceTool('get_item_price', { itemId: 4151 })).rejects.toThrow(
                'API request failed: 404'
            );
        });

        it('should handle 500 errors for graph', async () => {
            nock(RS3_GE_API)
                .get('/graph/4151.json')
                .reply(500, 'Internal Server Error');

            await expect(handleItemTool('get_item_graph', { itemId: 4151 })).rejects.toThrow(
                'API request failed: 500'
            );
        });

        it('should handle malformed JSON', async () => {
            nock(RS3_GE_API).get('/info.json').reply(200, 'invalid json');

            await expect(handlePriceTool('get_ge_info', {})).rejects.toThrow();
        });
    });

    describe('Invalid Tool Usage', () => {
        it('should throw error for unknown price tool', async () => {
            await expect(handlePriceTool('unknown_tool', {})).rejects.toThrow(
                'Unknown price tool: unknown_tool'
            );
        });
